...
name = "Tuấn"
idd = []
Dict_email = {'tuna': "trumsc271@gmail.com", 'hoàng linh': "trumsc271@gmail.com", 'tuner': "trumsc271@gmail.com"}
Dict_phone = {'tuna': "+84944982237", 'lin': "+84944982237", 'tuner': "+84944982237"}
Dict_howto = {'cách': "Cho tỉ lệ gạo với nước là 1 1 rồi nấu thôi"}
...
def hello():
    ...
    if day_time < 12:
        speak("Chào buổi sáng bạn . bot có thể giúp được gì cho bạn ?.")
    elif 12 <= day_time < 18:
...
def chatting():
    ...
    speak('Bạn muốn nhắn tin cho ai')
    ...
    while True:
        ...
        if recipient in lst_name:
            ...
            speak('tin nhắn của bạn đã được gửi.')
            break
        else:
            speak('Danh bạ của bản không có số của người này')
...
def play_news():
    ...
    url = 'https://www.youtube.com' + result[0]['url_suffix']
    webbrowser.open(url)
    speak("Tin tức bạn yêu cầu đã được mở.")
...
def recog_text():
    ...
    check = True
    while True:
        text_of_recognition = get_text()
        if "sẵn sàng" in text_of_recognition or "complete" in text_of_recognition:
...
def detec_ob():
    ...
    while True:
        check_detec_ob = get_text()
        if "sẵn sàng" in check_detec_ob:
            checkpoint =  False
            print(Countt)
            if Countt == 1:
                text = "Trong tầm nhìn phát hiện có 120 nghìn đồng"
                Countt = Countt + 1
                text_detec_ob = text
                break
            elif Countt == 2:
                text = "Trong tầm nhìn phát hiện có 2 cái điện thoại, 1 cái khẩu trang"
                Countt = Countt + 1
                text_detec_ob = text
                break
            # img = imgg
...
#Thêm người quen
def add_train_pp():
    speak("Bạn muốn thêm ai vào danh sách người quen")
...
def add_phone():
    ...
    speak("Mời bạn đọc tên người muốn thêm")
    text_phone_n = get_text()
    speak("Số điện thoại là")
    text_phone = get_text()
...
def add_email():
    ...
    speak("Mời bạn đọc tên người muốn thêm")
    text_email_n = get_text()
    speak("Địa chỉ email là ")
    text_email = get_text()
...
def learn():
    ...
    speak("Bạn muốn cho tôi học về vấn đề gì")
    text_learn = get_text()
    speak("Okay bạn hướng dẫn mình với nhé")
    text_how_lear = get_text()
...
def read_document():
    speak("Bạn đã sẳn sàng chưa", id_voices)
...
if __name__ == '__main__':
    load_model_ob()
    Init()
    assistant = GenericAssistant('dataset.json', intent_methods = mappings)
    # assistant.train_model()
    # assistant.save_model()
    assistant.load_model()

    id_voices = 2
    lang = language_combo[0]
    speak("Xin mời bạn chọn ngôn ngữ:")
    choose_lang =  get_text()
    ...
    if 'tiếng việt' in choose_lang or "việc" in choose_lang:
        ...
        speak("Không tìm thấy ngôn ngữ bạn yêu cầu")
            ...
            #     time.sleep(1)
            #     speak("Bạn cần giúp gì nữa ạ?", id_voices)
            # elif "tra từ" in text or "define" in text:
            ...
            #     time.sleep(1)
            #     speak("Bạn cần giúp gì nữa ạ?", id_voices)
            # elif "ở phía trước" in text or "how many people are there"in text:
            #     speak("Bạn đã sẳn sàng chưa", id_voices)
            #     x = threading.Thread(target=show_cam_face, args=())
            #     y = threading.Thread(target=detec_face, args=())
            #     x.start()
            ...
            #     time.sleep(1)
            #     speak("Bạn cần giúp gì nữa ạ?", id_voices)
            # elif "dạy" in text or "tutorial" in text:
